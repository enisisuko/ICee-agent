...
### v0.2.5 — LLMNodeExecutor 校验修复

**根本问题**：`packages/core/src/executor/builtins/LLMNodeExecutor.ts` 第 26 行强制 `if (!config?.provider || !config?.model) throw Error`，
